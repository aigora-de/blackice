...
Why ``kuang`` and not ``blackice``: the import name ``blackice`` is already taken
on PyPI by an unrelated project, which ships a top-level ``blackice`` package and
a ``blackice`` console script from the same entry point we would have used.
Installing both into one environment silently merges the two directories,
resolves the command to whichever won, and breaks the other's uninstall — so the
project is blackice and the artefact it ships is ``kuang``, after the intrusion
program that cuts through ICE.
"""
